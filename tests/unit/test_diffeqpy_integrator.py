...
from degym.integrators import TimeSpan, DiffeqpyIntegrator, DiffeqpyIntegratorConfig
...
def true_solution_rc(
    ...
    def system_dynamics(time: float, u_0: float) -> list[float]:
        ...
        i = (1 / resistance) * u_0 * np.exp(-time / tau)
        return [u, i]

    return system_dynamics


class RCDiffeqpySystemDynamicsFn(DiffeqpySystemDynamicsFn):
    ...
    def __call__(
        ...
        The chosen systel us an RC circuit:

            (1): du/dt = -u / tau with tau = RC
            (2): u(t) = R * i(t)

        Here, u(t) = u(t=0) * exp(-t/tau) and i(t) = 1/ R u(t=0) * exp(-t/tau)
        """
        resistance, capacity = parameters
        u, i = state
        derivative[0] = -u / (resistance * capacity)
        derivative[1] = resistance * i - u


@pytest.fixture
def rc_diffeqpy_dynamics_fn(
    resistance: float, capacity: float
) -> RCDiffeqpySystemDynamicsFn:
    return RCDiffeqpySystemDynamicsFn(capacity=capacity, resistance=resistance)
...
class RCState(State):
    ...
    def to_np_array(self) -> NDArray[np.floating]:
        return np.asarray([self.u, self.i])

    def __deepcopy__(self, memodict: dict) -> "RCState":
        state_copy = RCState(self.u, self.i)
...
def test_diffeqpy_integrate(
    ...
    resistance: float,
    capacity: float,
    rc_diffeqpy_dynamics_fn: DiffeqpySystemDynamicsFn,
) -> None:
    integrator = DiffeqpyIntegrator(
        system_dynamics=rc_diffeqpy_dynamics_fn,
        integrator_config=DiffeqpyIntegratorConfig(action_duration=duration),
    )

    i_0 = u_0 / resistance

    initial_state = RCState(u=u_0, i=i_0)
